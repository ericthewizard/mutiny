...
def format_sandbox(option, value):
    ...
    Examples:
        >>> # Set the plot title
        >>> import mutiny
        >>> mutiny.tplot_options('title', 'SWEA Data for Orbit 1563')

        >>> # Set the window size
        >>> mutiny.tplot_options('wsize', [1000,500])

    """
    option = option.lower()
    
    temp = tplot_utilities.set_tplot_options(option, value, mutiny.tplot_opt_glob)
    mutiny.tplot_opt_glob = temp
