...
def timebar(t, varname=None, databar=False, delete=False, color="black", thick=1, dash=False):
    ...
    >>> import mutiny
    >>> mutiny.timebar(1500249600, color='green')

    >>> # Place a dashed data bar at 5500 on the y axis
    >>> mutiny.timebar(5500, dash=True, databar=True)
    ...
    >>> mutiny.timebar([1451107201,1451117200,1451119999],'sgx',color='m',thick=5)
    """
    ...
    if varname is None:
        ...
        for i in range(num_bars):
            ...
            tbar["location"] = t[i]
            tbar["dimension"] = dim
            tbar["line_color"] = mutiny.tplot_utilities.rgb_color(color)[0]
            tbar["line_width"] = thick
            tbar["line_dash"] = dash_pattern
            for name in mutiny.data_quants:
                temp_data_quants = mutiny.data_quants[name]
    ...
    # if varname specified
        ...
        if not isinstance(varname, list):
            varname = [varname]
        for j in varname:
            if j not in mutiny.data_quants.keys():
                logging.info(str(j) + "is currently not in mutiny")
            else:
                num_bars = len(t)
                for i in range(num_bars):
                    tbar = {}
                    ...
                    tbar["line_width"] = thick
                    tbar["line_dash"] = dash_pattern
                    temp_data_quants = mutiny.data_quants[j]
                    if isinstance(temp_data_quants, dict):
                        # non-record varying variable
                        continue
                    temp_data_quants.attrs["plot_options"]["time_bar"].append(tbar)
