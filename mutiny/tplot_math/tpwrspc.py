import logging
import numpy as np
from mutiny import tnames, get_data, store_data, options
...
def tpwrspc(varname, newname=None, overwrite=False, noline=False, nohanning=False, bin=3, notperhz=False, trange=None, axis=0):
    ...
    It applies pwrspc to a mutiny variable and stores the result in a new mutiny variable.
    ...
    --------
        >>> import mutiny
        >>> import numpy as np
        >>> mutiny.store_data('a', data={'x': range(100), 'y': np.random.random(100)})
        >>> mutiny.tpwrspc('a')
        >>> mutiny.get_data('a_pwrspc')
    """
