...
def degap(
    ...
    --------
        ...
        >>> import mutiny
        >>> time = [mutiny.time_float("2020-01-01") + i for i in [1, 2, 3, 4, 5, 6, 9, 10, 11]]
        >>> y = [1, 2, 3, 4, 5, 6, 9, 10, 11]
        >>> mutiny.store_data("a", data={"x": time, "y": y})
        >>> degap("a", newname="b")
        >>> b = mutiny.get("b")
        >>> print(b)
    """
    ...
    #    gap_size = np.diff(mutiny.data_quants[tvar].coords['time']) This is in Nanoseconds, and causes a type mismatch with dt+margin
    #    new_tvar_index = mutiny.data_quants[tvar].coords['time']
    new_tvar_index = mutiny.get_data(tvar)[0]  # Unix time float64
    ...
    a = mutiny.data_quants[tvar].reindex({"time": new_index}, method=method)

    if newname is None:
        a.name = tvar
        a.attrs = copy.deepcopy(mutiny.data_quants[tvar].attrs)
        mutiny.data_quants[tvar] = copy.deepcopy(a)
    else:
