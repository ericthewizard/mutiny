...
def spec_mult(
    ...
    Examples
    --------
        >>> mutiny.store_data('h', data={'x':[0,4,8,12,16,19,21], 'y':[[8,1,1],[100,2,3],[4,2,47],[4,39,5],[5,5,99],[6,6,25],[7,-2,-5]],'v':[[1,1,50],[2,2,3],[100,4,47],[4,90,5],[5,5,99],[6,6,25],[7,7,-5]]})
        >>> mutiny.spec_mult('h')
        >>> print(mutiny.data_quants['h_specmult'].data)
    """
    ...
    if 'spec_bins' not in mutiny.data_quants[tvar].coords:
    ...
    mutiny.store_data(newname,data={'x': new_df.index,'y': new_df.values})
