...
def add_across(tvar,column_range=None,newname=None,new_tvar=None):
    ...
    --------
        ...
        >>> mutiny.add_across('d',newname='d_aa')
        >>> print(mutiny.data_quants['d_aa'].data)
        ...
        >>> mutiny.add_across('b',column_range=[[1,2],[3,4]],newname='b_aap')
        >>> print(mutiny.data_quants['b_aap'].data)
    """
    ...
    if 'spec_bins' in mutiny.data_quants[tvar].coords:
        d, s = mutiny.tplot_utilities.convert_tplotxarray_to_pandas_dataframe(tvar)
    else:
        d = mutiny.tplot_utilities.convert_tplotxarray_to_pandas_dataframe(tvar, no_spec_bins=True)
        s = None
    ...
    #store added data
    if s is None:
        mutiny.store_data(newname,data={'x':time, 'y':np.transpose(data)})
    else:
        mutiny.store_data(newname, data={'x': time, 'y':np.transpose(data), 'v': np.transpose(spec_data)})

    #mutiny.data_quants[new_tvar].attrs = copy.deepcopy(mutiny.data_quants[tvar].attrs)
