...
def clean_spikes(names, nsmooth=10, thresh=0.3, sub_avg=False,
    ...
    if len(old_names) < 1:
        logging.error('clean_spikes error: No mutiny names were provided.')
        return
    ...
    for old_idx, old in enumerate(old_names):
        ...
        if sub_avg:
            ...
            mutiny.tplot_copy(new, tmp)

        # Find spikes
        tmps = tmp + '-s'
        tsmooth(tmp, newname=tmps, width=nsmooth)
        ds0 = mutiny.get_data(tmps)  # smoothed out values
        ds = ds0[1]
        dor0 = mutiny.get_data(tmp)  # original values
        d0 = dor0[1]
        ...
        # mutiny.data_quants[new] = d
        mutiny.replace_data(new, dn)

        # remove temp data
        del mutiny.data_quants[tmp]
        del mutiny.data_quants[tmps]
