...
def format_sandbox(option, value):
    ...
    Examples:
        >>> # Set the plot title
        >>> import mutinyplot
        >>> mutinyplot.tplot_options('title', 'SWEA Data for Orbit 1563')

        >>> # Set the window size
        >>> mutinyplot.tplot_options('wsize', [1000,500])

    """
    option = option.lower()
    
    temp = tplot_utilities.set_tplot_options(option, value, mutinyplot.tplot_opt_glob)
    mutinyplot.tplot_opt_glob = temp
