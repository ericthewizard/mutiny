...
def link(names, link_name, link_type='alt'):
    ...
    for i in names:
        if i not in mutinyplot.data_quants.keys():
            logging.info(str(i) + " is not currently in mutinyplot.")
            return
        
        if isinstance(i,int):
            i = list(mutinyplot.data_quants.keys())[i-1]

        if isinstance(mutinyplot.data_quants[i], dict):  # non-record varying variable
            continue
        mutinyplot.data_quants[mutinyplot.data_quants[i].name].attrs['plot_options']['links'][link_type] = link_name
