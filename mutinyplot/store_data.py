...
def store_data(name, data=None, delete=False, newname=None, attr_dict={}):
    ...
    ----------
        ...
        newname: str
            ...
            Default: False
        attr_dict: dict
            A dictionary object of attributes (these do not affect routines in mutinyplot, this is merely to keep metadata alongside the file)
    ...
    --------
        ...
        >>> mutinyplot.store_data("Variable2", data={'x':x_data, 'y':y_data})
        
        >>> # Store a spectrogram
        >>> x_data = [1,2,3]
        >>> y_data = [ [1,2,3] , [4,5,6], [7,8,9] ]
        >>> v_data = [1,2,3]
        >>> mutinyplot.store_data("Variable3", data={'x':x_data, 'y':y_data, 'v':v_data})
        
        >>> # Combine two different line plots
        >>> mutinyplot.store_data("Variable1and2", data=['Variable1', 'Variable2'])
        
        >>> #Rename TVar
        >>> mutinyplot.store_data('a', data={'x':[0,4,8,12,16], 'y':[1,2,3,4,5]})
        >>> mutinyplot.store_data('a',newname='f')
    ...
    if newname is not None:
        mutinyplot.tplot_rename(name, newname)
        return True

    # if isinstance(data, str):
    #     mutinyplot.data_quants[name] = {'name': name, 'data': data}
    #     return True
    if isinstance(data, str):
        data = data.split(' ')
    ...
    if isinstance(data, list):
        ...
        mutinyplot.data_quants[name] = copy.deepcopy(mutinyplot.data_quants[base_data[0]])
        mutinyplot.data_quants[name].attrs = copy.deepcopy(mutinyplot.data_quants[base_data[0]].attrs)
        mutinyplot.data_quants[name].name = name
        mutinyplot.data_quants[name].attrs['plot_options']['overplots'] = base_data[1:]
        mutinyplot.data_quants[name].attrs['plot_options']['overplots_mpl'] = base_data
        ...
        mutinyplot.data_quants[name].attrs['plot_options']['yaxis_opt'] = {}
        mutinyplot.data_quants[name].attrs['plot_options']['zaxis_opt'] = {}
        mutinyplot.data_quants[name].attrs['plot_options']['line_opt'] = {}
        mutinyplot.data_quants[name].attrs['plot_options']['extras'] = {}
        return True

    # if the data table doesn't contain an 'x', assume this is a non-record varying variable
    if 'x' not in data.keys():
        values = np.array(data.pop('y'))
        mutinyplot.data_quants[name] = {'data': values}
        mutinyplot.data_quants[name]['name'] = name
        return True
    ...
    mutinyplot.data_quants[name] = temp

    mutinyplot.data_quants[name].attrs['plot_options']['yaxis_opt']['y_range'] = utilities.get_y_range(temp)
...
def _get_base_tplot_vars(name,data):
    ...
    if not isinstance(data, list):
        data = [data]
    for var in data:
        if var not in mutinyplot.data_quants:
            logging.warning('store_data: Pseudovariable %s component %s not found, skipping', name, var)
        elif isinstance(mutinyplot.data_quants[var].data, list):
            base_vars += _get_base_tplot_vars(name,mutinyplot.data_quants[var].data)
        else:
            base_vars += [var]
    return base_vars
...
def store(name, data=None, delete=False, newname=None, metadata={}):
    ...
    Examples:
        ...
        >>> x_data = [1,2,3,4,5]
        >>> y_data = [1,2,3,4,5]
        >>> mutinyplot.store("Variable1", data={'x':x_data, 'y':y_data})

        >>> # Store a two lines
        >>> x_data = [1,2,3,4,5]
        >>> y_data = [[1,5],[2,4],[3,3],[4,2],[5,1]]
        >>> mutinyplot.store("Variable2", data={'x':x_data, 'y':y_data})

        >>> # Store a spectrogram
        >>> x_data = [1,2,3]
        >>> y_data = [ [1,2,3] , [4,5,6], [7,8,9] ]
        >>> v_data = [1,2,3]
        >>> mutinyplot.store("Variable3", data={'x':x_data, 'y':y_data, 'v':v_data})

        >>> # Combine two different line plots
        >>> mutinyplot.store("Variable1and2", data=['Variable1', 'Variable2'])

        >>> #Rename TVar
        >>> mutinyplot.store('a', data={'x':[0,4,8,12,16], 'y':[1,2,3,4,5]})
        >>> mutinyplot.store('a',newname='f')
    """
