...
def tkm2re(name, km=False, newname=None, suffix=''):
    ...
    --------
        ...
        >>> mutinyplot.tkm2re('a', km = True)
        >>> print(mutinyplot.data_quants['a_km'].data)

    """
    km_in_re = 6371.2

    names = mutinyplot.tnames(name)
    ...
    for in_tvar, out_tvar in zip(names, newname):
        data = mutinyplot.get_data(in_tvar)
        metadata = mutinyplot.get_data(in_tvar, metadata=True)
        ...
        saved = mutinyplot.store_data(out_tvar, data={'x': data.times, 'y': data_out}, attr_dict=metadata)
        ...
        yaxis_opt = mutinyplot.data_quants[out_tvar].attrs['plot_options'].get('yaxis_opt')

        if yaxis_opt is not None:
            subtitle = yaxis_opt.get('axis_subtitle')
            if subtitle is not None:
                if km == False:
                    new_subtitle = mutinyplot.data_quants[out_tvar].attrs['plot_options']['yaxis_opt']['axis_subtitle'].lower().replace('km', 'Re')
                else:
                    new_subtitle = mutinyplot.data_quants[out_tvar].attrs['plot_options']['yaxis_opt']['axis_subtitle'].lower().replace('re', 'km')
                mutinyplot.data_quants[out_tvar].attrs['plot_options']['yaxis_opt']['axis_subtitle'] = new_subtitle
