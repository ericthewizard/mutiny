...
def split_vec(
    ...
    --------
        ...
        >>> mutinyplot.tplot_math.split_vec('b')
        >>> print(mutinyplot.data_quants['b_0'].values)
        >>> print(mutinyplot.data_quants['b_1'].values)
        >>> print(mutinyplot.data_quants['b_5'].values)
    """
    ...
    if tvar not in mutinyplot.data_quants:
    ...
    alldata = mutinyplot.get_data(tvar)
    time = alldata[0]
    data = alldata[1]
    dim = data.shape
    metadata = mutinyplot.get_data(tvar, metadata=True)
    ...
    for i in columns:
        ...
        if not mutinyplot.store_data(split_name, data=data_for_tplot, attr_dict=metadata):
            raise Exception(f"Failed to store {split_name} in mutinyplot.")
