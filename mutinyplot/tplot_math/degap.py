...
def degap(
    ...
    --------
        ...
        >>> import mutinyplot
        >>> time = [mutinyplot.time_float("2020-01-01") + i for i in [1, 2, 3, 4, 5, 6, 9, 10, 11]]
        >>> y = [1, 2, 3, 4, 5, 6, 9, 10, 11]
        >>> mutinyplot.store_data("a", data={"x": time, "y": y})
        >>> degap("a", newname="b")
        >>> b = mutinyplot.get("b")
        >>> print(b)
    """
    ...
    #    gap_size = np.diff(mutinyplot.data_quants[tvar].coords['time']) This is in Nanoseconds, and causes a type mismatch with dt+margin
    #    new_tvar_index = mutinyplot.data_quants[tvar].coords['time']
    new_tvar_index = mutinyplot.get_data(tvar)[0]  # Unix time float64
    ...
    a = mutinyplot.data_quants[tvar].reindex({"time": new_index}, method=method)

    if newname is None:
        a.name = tvar
        a.attrs = copy.deepcopy(mutinyplot.data_quants[tvar].attrs)
        mutinyplot.data_quants[tvar] = copy.deepcopy(a)
    else:
        if "spec_bins" in a.coords:
            ...
            mutinyplot.data_quants[newname].attrs = copy.deepcopy(
                mutinyplot.data_quants[tvar].attrs
            )
        else:
