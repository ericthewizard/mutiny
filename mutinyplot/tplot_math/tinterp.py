...
def tinterp(tvar1,tvar2,replace=False):
    ...
    --------
        ...
        >>> mutinyplot.tinterp('a','c')
        >>> print(mutinyplot.data_quants['c_interp'].data)
    """
    new_tvar2 = mutinyplot.data_quants[tvar2].interp_like(mutinyplot.data_quants[tvar1])

    if replace:
        mutinyplot.data_quants[tvar2] = new_tvar2
        return
    else:
        mutinyplot.data_quants[tvar1 + '_tinterp'] = copy.deepcopy(new_tvar2)
        mutinyplot.data_quants[tvar1 + '_tinterp'].attrs = copy.deepcopy(new_tvar2.attrs)
        mutinyplot.data_quants[tvar1 + '_tinterp'].name = tvar1 + '_tinterp'
