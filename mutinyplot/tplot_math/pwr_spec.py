...
def pwr_spec(tvar, nbp=256, nsp=128, newname=None):
    ...
    --------
        >>> import mutinyplot
        >>> import math
        >>> time = [mutinyplot.time_float("2020-01-01") + i for i in range(10000)]
        >>> quantity = [math.sin(i) for i in range(10000)]
        >>> mutinyplot.store_data("dp", data={"x": time, "y": quantity})
        >>> mutinyplot.pwr_spec("dp", name="dp_pwrspec")
        >>> mutinyplot.tplot("dp_pwrspec")
    """

    d = mutinyplot.get_data(tvar)
    x, y = d[0], d[1]
    ...
    mutinyplot.store_data(name, data={"x": x_new, "y": pxx_new, "v": f_new})
    mutinyplot.options(name, "spec", 1)
    mutinyplot.options(name, "zlog", 1)
    mutinyplot.options(name, "ylog", 1)
