...
def time_clip(
    ...
    -------
    list of str
        Returns a list of mutinyplot variables created or changed
    ...
    -------
        ...
        >>> mutinyplot.store_data("a", data={"x": time1, "y": [[1, 2, 3],[2, 3, 4],[3, 4, 5],[4, 5, 6],[5, 6,7]]})
        >>> time_start=time1[0]
        >>> time_end=time1[2]
        >>> mutinyplot.time_clip('a',time_start,time_end)
        >>> ac = mutinyplot.get_data('a-tclip')
        >>> print(ac)

    """
    if len(names) < 1:
        logging.warning('time_clip: no mutinyplot variables specified')
        return
    ...
    old_names = mutinyplot.tnames(names)

    if len(old_names) < 1:
        logging.warning('time_clip: No mutinyplot variables matching '+str(names))
        return
    ...
    for j in range(len(old_names)):
        if old_names[j] != n_names[j]:
            mutinyplot.tplot_copy(old_names[j], n_names[j])

        alldata = mutinyplot.get_data(n_names[j])
        metadata = mutinyplot.get_data(n_names[j], metadata=True)
        ...
        new_time = mutinyplot.time_float(time)
        new_time_start = mutinyplot.time_float(time_start)
        new_time_end = mutinyplot.time_float(time_end)
        ...
        tmp_q = mutinyplot.data_quants[n_names[j]]
        ...
        try:
            if 'v1' in tmp_q.coords.keys() and\
                ...
                mutinyplot.store_data(n_names[j], data={
