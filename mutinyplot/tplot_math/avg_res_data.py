...
def avg_res_data(tvar,res,newname=None,new_tvar=None):
    ...
    --------
        ...
        >>> mutinyplot.avg_res_data('d',2,'d2res')
        >>> print(mutinyplot.data_quants['d'].values)
    """
    ...
    tvar_new = mutinyplot.data_quants[tvar].coarsen(time=res, boundary='trim').mean()
    tvar_new.name = mutinyplot.data_quants[tvar].name
    tvar_new.attrs = copy.deepcopy(mutinyplot.data_quants[tvar].attrs)

    if newname is None:
        mutinyplot.data_quants[tvar] = newname
    else:
        if 'spec_bins' in mutinyplot.data_quants[tvar].coords:
            mutinyplot.store_data(newname, data={'x': tvar_new.coords['time'].values, 'y': tvar_new.values,
