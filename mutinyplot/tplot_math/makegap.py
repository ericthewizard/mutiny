...
def makegap(var_data, dt=None, margin=0.0, func="nan"):
    ...
    --------
        ...
        >>> a = mutinyplot.get_data("a")
        >>> print(a)
        >>> b = mutinyplot.makegap(a)
        >>> print(b)
    ...
    #x = mutinyplot.time_float(var_data.times)
    x = np.int64(var_data.times)/1e9
    if var_data.y.ndim == 1:
        mutinyplot.store_data("makegap_tmp", data={"x": x, "y": var_data.y})
    ...
    mutinyplot.degap("makegap_tmp", dt=dt, margin=margin, func=func)
    # and return the getdata result
    var_data_out = mutinyplot.get_data("makegap_tmp", dt=True)
    mutinyplot.del_data("makegap_tmp")
    return var_data_out
