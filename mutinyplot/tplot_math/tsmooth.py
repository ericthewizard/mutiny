...
def smooth(data, width=10, preserve_nans=None):
    ...
    -------
        >>> import mutinyplot
        >>> import numpy as np
        >>> print(mutinyplot.smooth(np.random.random(100)))
...
def tsmooth(names, width=10, median=None, preserve_nans=None,
    ...
    -------
        >>> import mutinyplot
        >>> import numpy as np
        >>> mutinyplot.store_data('a', data={'x': range(100), 'y': np.random.random(100)})
        >>> mutinyplot.tsmooth('a')
        >>> print(mutinyplot.data_quants['a-s'].data)
    """
    old_names = mutinyplot.tnames(names)

    if len(old_names) < 1:
        logging.error('tsmooth error: No mutinyplot names were provided.')
        return
    ...
    for i, old in enumerate(old_names):
        ...
        data = mutinyplot.data_quants[new].values
        ...
        mutinyplot.data_quants[new].values = data
