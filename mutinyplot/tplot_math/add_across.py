...
def add_across(tvar,column_range=None,newname=None,new_tvar=None):
    ...
    --------
        ...
        >>> mutinyplot.add_across('d',newname='d_aa')
        >>> print(mutinyplot.data_quants['d_aa'].data)
        ...
        >>> mutinyplot.add_across('b',column_range=[[1,2],[3,4]],newname='b_aap')
        >>> print(mutinyplot.data_quants['b_aap'].data)
    """
    ...
    if 'spec_bins' in mutinyplot.data_quants[tvar].coords:
        d, s = mutinyplot.tplot_utilities.convert_tplotxarray_to_pandas_dataframe(tvar)
    else:
        d = mutinyplot.tplot_utilities.convert_tplotxarray_to_pandas_dataframe(tvar, no_spec_bins=True)
        s = None
    ...
    #store added data
    if s is None:
        mutinyplot.store_data(newname,data={'x':time, 'y':np.transpose(data)})
    else:
        mutinyplot.store_data(newname, data={'x': time, 'y':np.transpose(data), 'v': np.transpose(spec_data)})

    #mutinyplot.data_quants[new_tvar].attrs = copy.deepcopy(mutinyplot.data_quants[tvar].attrs)
