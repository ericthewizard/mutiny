...
def clip(tvar,ymin,ymax,newname=None,new_tvar=None):
    ...
    --------
        ...
        >>> mutinyplot.store_data('d', data={'x':[2,5,8,11,14,17,21], 'y':[[1,1],[2,2],[100,100],[4,4],[5,5],[6,6],[7,7]]})
        >>> mutinyplot.clip('d',2,6,'e')
        >>> print(mutinyplot.data_quants['e'].data)
    """
    ...
    if len(tn) > 1:
        for j in range(len(tn)):
            mutinyplot.clip(tn[j],ymin,ymax)
        return


    a = copy.deepcopy(mutinyplot.data_quants[tvar].where(mutinyplot.data_quants[tvar] >= ymin))
