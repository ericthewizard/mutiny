...
def clean_spikes(names, nsmooth=10, thresh=0.3, sub_avg=False,
    ...
    if len(old_names) < 1:
        logging.error('clean_spikes error: No mutinyplot names were provided.')
        return
    ...
    for old_idx, old in enumerate(old_names):
        ...
        # Create new
        if old != new:
            mutinyplot.tplot_copy(old, new)

        # Perform subtract_average or just copy the values
        if sub_avg:
            subtract_average(new, newname=tmp)
        else:
            mutinyplot.tplot_copy(new, tmp)

        # Find spikes
        tmps = tmp + '-s'
        tsmooth(tmp, newname=tmps, width=nsmooth)
        ds0 = mutinyplot.get_data(tmps)  # smoothed out values
        ds = ds0[1]
        dor0 = mutinyplot.get_data(tmp)  # original values
        d0 = dor0[1]
        ...
        # mutinyplot.data_quants[new] = d
        mutinyplot.replace_data(new, dn)

        # remove temp data
        del mutinyplot.data_quants[tmp]
        del mutinyplot.data_quants[tmps]
