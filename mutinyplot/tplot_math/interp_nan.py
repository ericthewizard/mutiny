...
def interp_nan(tvar, newname=None, new_tvar=None, s_limit=None):
    ...
    >>> mutinyplot.interp_nan('e','e_nonan',s_limit=5)
    >>> print(mutinyplot.data_quants['e_nonan'].values)
    """
    ...
    x = mutinyplot.data_quants[tvar].interpolate_na(dim="time", limit=s_limit)
    x.attrs = copy.deepcopy(mutinyplot.data_quants[tvar].attrs)

    if newname is None:
        mutinyplot.data_quants[tvar] = x
        x.name = tvar
    else:
        mutinyplot.data_quants[newname] = x
        mutinyplot.data_quants[newname].name = newname
