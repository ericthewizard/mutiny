...
def deflag(tvar, flag=None, newname=None, new_tvar=None, method=None, fillval=None):
    ...
    --------
        ...
        >>> mutinyplot.store_data('d', data={'x':[2,5,8,11,14,17,21], 'y':[[1,1],[2,2],[100,4],[4,90],[5,5],[6,6],[7,7]]})
        >>> # Remove any instances of [100,90,7,2,57] from 'd', store in 'e'.
        >>> mutinyplot.deflag('d',[100,90,7,2,57],newname='e')
    ...
    tn = mutinyplot.tnames(tvar)
    if len(tn) > 1:
        for j in range(len(tn)):
            mutinyplot.deflag(tn[j], flag, method=method, fillval=fillval)
        return
    ...
    if method == 'remove_nan':  # this is different from the other methods, which retain all time intervals
        a = copy.deepcopy(mutinyplot.get_data(tvar))
        alen = len(a)
        ...
        if newname is None:
            if alen == 2:
                mutinyplot.store_data(tvar, data={'x': new_time, 'y': new_data})
            else:
                mutinyplot.store_data(tvar, data={'x': new_time, 'y': new_data, 'v': new_v})
        else:
            if alen == 2:
                mutinyplot.store_data(newname, data={'x': new_time, 'y': new_data})
            else:
                mutinyplot.store_data(newname, data={'x': new_time, 'y': new_data, 'v': new_v})
            mutinyplot.data_quants[newname].attrs = copy.deepcopy(mutinyplot.data_quants[tvar].attrs)
        ...
        if newname is None:
            if alen == 2:
                mutinyplot.store_data(tvar, data={'x': time, 'y': data})
            else:
                mutinyplot.store_data(tvar, data={'x': time, 'y': data, 'v': v})
        else:
            if alen == 2:
                mutinyplot.store_data(newname, data={'x': time, 'y': data})
            else:
                mutinyplot.store_data(newname, data={'x': time, 'y': data, 'v': v})
                mutinyplot.data_quants[newname].attrs = copy.deepcopy(mutinyplot.data_quants[tvar].attrs)
    else:  # any other option includes method=None, replace flags with NaN
        nf = len(flag)
        a = copy.deepcopy(mutinyplot.data_quants[tvar].where(mutinyplot.data_quants[tvar] != flag[0]))
        if nf > 1:
            for j in range(nf):
                a = copy.deepcopy(a.where(a != flag[j]))
        if newname is None:
            a.name = tvar
            mutinyplot.data_quants[tvar] = a
        else:
            if 'spec_bins' in a.coords:
                mutinyplot.store_data(newname, data={'x': a.coords['time'], 'y': a.values, 'v': a.coords['spec_bins']})
                mutinyplot.data_quants[newname].attrs = copy.deepcopy(mutinyplot.data_quants[tvar].attrs)
            else:
                mutinyplot.store_data(newname, data={'x': a.coords['time'], 'y': a.values})
                mutinyplot.data_quants[newname].attrs = copy.deepcopy(mutinyplot.data_quants[tvar].attrs)
