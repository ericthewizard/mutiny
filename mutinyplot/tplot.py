...
from mutinyplot.wildcard_routines import tplot_wildcard_expand, tname_byindex
...
def tplot(variables, var_label=None,
    ...
    style = mutinyplot.tplot_opt_glob.get('style')
    ...
    for var_idx, variable in enumerate(variables):
        if mutinyplot.data_quants.get(variable) is None:
            continue
        panel_size = mutinyplot.data_quants[variable].attrs['plot_options']['extras'].get('panel_size')
        if panel_size is not None:
            panel_sizes[var_idx] = panel_size

    if xsize is None:
        xsize = mutinyplot.tplot_opt_glob.get('xsize')
        if xsize is None:
            xsize = 12

    if ysize is None:
        ysize = mutinyplot.tplot_opt_glob.get('ysize')
        if ysize is None:
            if num_panels > 4:
                ysize = 8
            else:
    ...
    plot_title = mutinyplot.tplot_opt_glob['title_text']
    axis_font_size = mutinyplot.tplot_opt_glob.get('axis_font_size')
    vertical_spacing = mutinyplot.tplot_opt_glob.get('vertical_spacing')
    xmargin = mutinyplot.tplot_opt_glob.get('xmargin')
    ymargin = mutinyplot.tplot_opt_glob.get('ymargin')
    zrange = [None, None]
    ...
    for idx, variable in enumerate(variables):
        var_data_org = mutinyplot.get_data(variable, dt=True)
        var_metadata = mutinyplot.get_data(variable, metadata=True)

        #Check for a 3d variable, call reduce_spec_dataset
        if hasattr(var_data_org, 'v1') and hasattr(var_data_org, 'v2'):
            temp_dq = mutinyplot.tplot_utilities.reduce_spec_dataset(name=variable)
            var_data_org = mutinyplot.get_data(variable, dt=True, data_quant_in=temp_dq)
        ...
        var_quants = mutinyplot.data_quants[variable]
        ...
        if isinstance(var_data, list) or isinstance(var_data, str) or pseudo_var:
            ...
            if idx > 0:
                mutinyplot.tplot_opt_glob['title_text'] = ''
            ...
            for pseudo_idx, var in enumerate(pseudo_vars):
                ...
                trace_count_thisvar = mutinyplot.count_traces(var)
            ...
            if idx > 0:
                mutinyplot.tplot_opt_glob['title_text'] = plot_title

            continue

        # set the figure title
        if idx == 0 and plot_title != '':
            if 'title_size' in mutinyplot.tplot_opt_glob:
                title_size = mutinyplot.tplot_opt_glob['title_size']
        ...
        if plot_extras.get('data_gap') is not None and plot_extras.get('data_gap') > 0:
            var_data = mutinyplot.makegap(var_data, dt = plot_extras.get('data_gap'))
        else:
            if mutinyplot.tplot_opt_glob['data_gap'] is not None and mutinyplot.tplot_opt_glob['data_gap'] > 0:
                var_data = mutinyplot.makegap(var_data, dt = mutinyplot.tplot_opt_glob['data_gap'])

        # set the x-axis range, if it was set with xlim or tlimit
        if mutinyplot.tplot_opt_glob.get('x_range') is not None:
            x_range = mutinyplot.tplot_opt_glob['x_range']  # Seconds since epoch
        ...
        char_size = mutinyplot.tplot_opt_glob.get('charsize')
        ...
        if mutinyplot.data_quants[variable].attrs['plot_options'].get('time_bar') is not None:
            time_bars = mutinyplot.data_quants[variable].attrs['plot_options']['time_bar']
        ...
        if mutinyplot.data_quants[variable].attrs['plot_options'].get('highlight_intervals') is not None:
            highlight_intervals = mutinyplot.data_quants[variable].attrs['plot_options']['highlight_intervals']
        ...
        # add annotations
        if mutinyplot.tplot_opt_glob.get('annotations') is not None:
            annotations = mutinyplot.tplot_opt_glob['annotations']
    ...
    if var_label is not None:
        ...
        for label in var_label:
            ...
            label_data = mutinyplot.get_data(label, xarray=True, dt=True)
            ...
            ytitle = mutinyplot.data_quants[label].attrs['plot_options']['yaxis_opt']['axis_label']
    ...
    for idx, variable in enumerate(variables):
        if mutinyplot.data_quants.get(variable) is None:
            continue
        plot_extras = mutinyplot.data_quants[variable].attrs['plot_options']['extras']
...
def mouse_move_slice(event, slice_axes, slice_plot):
    ...
    try:
        data = mutinyplot.get_data(event.inaxes.var_name)
    ...
    yaxis_options = mutinyplot.data_quants[event.inaxes.var_name].attrs['plot_options']['yaxis_opt']
    zaxis_options = mutinyplot.data_quants[event.inaxes.var_name].attrs['plot_options']['zaxis_opt']
    ...
    slice_yaxis_opt = mutinyplot.data_quants[event.inaxes.var_name].attrs['plot_options'].get('slice_yaxis_opt')
