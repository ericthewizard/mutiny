...
def compare_versions():
    ...
    mutinyplot_url = "https://pypi.python.org/pypi/mutinyplot/json"
    try:
        pt_pypi_vn = sorted(requests.get(mutinyplot_url).json()['releases'])
    except:
        return
    ...
    if old_flag == 1:
        print("PyPI mutinyplot Version")
        print(pr1)
        print("Your mutinyplot Version in " + dir_path)
        print(pr2)
        print("")
        print('****************************** WARNING! ******************************')
        print('*                                                                    *')
        print('*          You are running an outdated version of mutinyplot.           *')
...
def timebar_delete(t, varname=None, dim='height'):
    if varname is None:
        for name in mutinyplot.data_quants:
            if isinstance(mutinyplot.data_quants[name], dict):
                # non-record varying variable
                continue
            try:
                list_timebars = mutinyplot.data_quants[name].attrs['plot_options']['time_bar']
            except KeyError:
                continue
            elem_to_delete = []
            ...
            mutinyplot.data_quants[name].attrs['plot_options']['time_bar'] = list_timebars
    else:
        ...
        for i in varname:
            ...
            if isinstance(mutinyplot.data_quants[i], dict):
                ...
                continue
            try:
                list_timebars = mutinyplot.data_quants[i].attrs['plot_options']['time_bar']
            except KeyError:
                continue
            elem_to_delete = []
            for elem in list_timebars:
            ...
            # mutinyplot.data_quants[i].attrs['plot_options']['time_bar']
    return
...
def get_available_qt_window(name='Plot'):
    # Delete old windows
    for n, w in zip(mutinyplot.mutinyplotWindow_names, mutinyplot.mutinyplotWindows):
        if not w.isVisible():
            del w
            del n

    # Add a new one to the list
    mutinyplot.mutinyplotWindows.append(mutinyplot.PlotWindow())
    mutinyplot.mutinyplotWindow_names.append(name)

    # Return the latest window
    return mutinyplot.mutinyplotWindows[-1]
...
def get_spec_data(names):
    ...
    valid_vars = list()
    for n in names:
        if 'spec_bins' in mutinyplot.data_quants[n].coords:
            valid_vars.append(n)
    return valid_vars
...
def get_spec_slicer_axis_types(names):
    ...
    for n in names:
        if 'spec_bins' in mutinyplot.data_quants[n].coords:
            zlabel = mutinyplot.data_quants[n].attrs['plot_options']['zaxis_opt']['axis_label']
            ylabel = mutinyplot.data_quants[n].attrs['plot_options']['yaxis_opt']['axis_label']

            if 'xi_axis_type' in mutinyplot.data_quants[n].attrs['plot_options']['interactive_xaxis_opt'].keys():
                xtype_interactive = mutinyplot.data_quants[n].attrs['plot_options']['interactive_xaxis_opt']['xi_axis_type']
            elif 'y_axis_type' in mutinyplot.data_quants[n].attrs['plot_options']['yaxis_opt']:
                xtype_interactive = mutinyplot.data_quants[n].attrs['plot_options']['yaxis_opt']['y_axis_type']
            else:
                xtype_interactive = 'log'

            if 'yi_axis_type' in mutinyplot.data_quants[n].attrs['plot_options']['interactive_yaxis_opt'].keys():
                ytype_interactive = mutinyplot.data_quants[n].attrs['plot_options']['interactive_yaxis_opt']['yi_axis_type']
            elif 'z_axis_type' in mutinyplot.data_quants[n].attrs['plot_options']['zaxis_opt']:
                ytype_interactive = mutinyplot.data_quants[n].attrs['plot_options']['zaxis_opt']['z_axis_type']
            else:
...
def set_spec_slice_x_range(var, x_axis_log, plot):
    ...
    if 'xi_range' in mutinyplot.data_quants[var].attrs['plot_options']['interactive_xaxis_opt']:
        if x_axis_log:
            plot.setXRange(np.log10(mutinyplot.data_quants[var].attrs['plot_options']['interactive_xaxis_opt']['xi_range'][0]),
                           np.log10(mutinyplot.data_quants[var].attrs['plot_options']['interactive_xaxis_opt']['xi_range'][1]),
                           padding=0)
        elif not x_axis_log:
            plot.setXRange(mutinyplot.data_quants[var].attrs['plot_options']['interactive_xaxis_opt']['xi_range'][0],
                           mutinyplot.data_quants[var].attrs['plot_options']['interactive_xaxis_opt']['xi_range'][1], padding=0)
...
def set_spec_slice_y_range(var, y_axis_log, plot):
    ...
    if 'yi_range' in mutinyplot.data_quants[var].attrs['plot_options']['interactive_yaxis_opt']:
        if y_axis_log:
            plot.setYRange(np.log10(mutinyplot.data_quants[var].attrs['plot_options']['interactive_yaxis_opt']['yi_range'][0]),
                           np.log10(mutinyplot.data_quants[var].attrs['plot_options']['interactive_yaxis_opt']['yi_range'][1]),
                           padding=0)
        elif not y_axis_log:
            plot.setYRange(mutinyplot.data_quants[var].attrs['plot_options']['interactive_yaxis_opt']['yi_range'][0],
                           mutinyplot.data_quants[var].attrs['plot_options']['interactive_yaxis_opt']['yi_range'][1], padding=0)
...
def convert_tplotxarray_to_pandas_dataframe(name, no_spec_bins=False):
    ...
    matrix = mutinyplot.data_quants[name].values
    while len(matrix.shape) > 3:
        matrix = np.nansum(matrix, 3)
    if len(matrix.shape) == 3:
        matrix = np.nansum(matrix, 1)
    return_data = pd.DataFrame(matrix)
    return_data = return_data.set_index(pd.Index(mutinyplot.data_quants[name].coords['time'].values))

    if no_spec_bins:
        return return_data

    if 'spec_bins' in mutinyplot.data_quants[name].coords:
        spec_bins = pd.DataFrame(mutinyplot.data_quants[name].coords['spec_bins'].values)
        if len(mutinyplot.data_quants[name].coords['spec_bins'].shape) == 1:
            spec_bins = spec_bins.transpose()
        else:
            spec_bins = spec_bins.set_index(pd.Index(mutinyplot.data_quants[name].coords['time'].values))
...
def return_interpolated_link_dict(dataset, types):
    ...
    for t in types:
        ...
        link = mutinyplot.data_quants[dataset.attrs['plot_options']['links'][t]]
...
def reduce_spec_dataset(tplot_dataset=None, name=None):
    ...
    if tplot_dataset is not None:
        ...
        da = copy.deepcopy(mutinyplot.data_quants[name])
    else:
        return
...
def highlight(variables=None, range=None, color='gray', alpha=0.2, fill=True, edgecolor=None, facecolor=None, hatch=None, delete=False):
    ...
    for variable in variables:
        if delete:
            mutinyplot.data_quants[variable].attrs['plot_options']['highlight_intervals'] = None
            continue
        if range is None:
            mutinyplot.data_quants[variable].attrs['plot_options']['highlight_intervals'] = None
            continue
        interval = {'location': range,
                    ...
                    'facecolor': facecolor,
                    'hatch': hatch}
        if mutinyplot.data_quants[variable].attrs['plot_options'].get('highlight_intervals') is None:
            mutinyplot.data_quants[variable].attrs['plot_options']['highlight_intervals'] = [interval]
        else:
            mutinyplot.data_quants[variable].attrs['plot_options']['highlight_intervals'].append(interval)
...
def annotate(text=None, position=None,
    ...
    """
    if delete:
        mutinyplot.tplot_opt_glob['annotations'] = None
        return
    ...
    if mutinyplot.tplot_opt_glob.get('annotations') is None:
        mutinyplot.tplot_opt_glob['annotations'] = [annotations]
    else:
        mutinyplot.tplot_opt_glob['annotations'].append(annotations)