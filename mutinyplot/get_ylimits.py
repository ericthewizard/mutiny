...
def get_ylimits(name, trg=None):
    ...
    Returns
    -------
    ymin : int, float, numeric type of mutinyplot data
        The minimum value of y.
    ymax : int, float, numeric type of mutinyplot data
    ...
    >>> y1, y2 = mutinyplot.get_ylimits("Variable1")
    """

    if isinstance(name, int):
        name = list(mutinyplot.data_quants.keys())[name-1]
    ...
    for i in range(name_num):
        ...
        if name[i] not in mutinyplot.data_quants.keys():
            logging.info(str(name[i]) + " is currently not in mutinyplot.")
            return
        y = mutinyplot.data_quants[name[i]]
