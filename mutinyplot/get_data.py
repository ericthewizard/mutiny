...
def get_data(name, xarray=False, metadata=False, dt=False, units=False, data_quant_in=None):
    ...
    --------
        ...
        >>> time, data = mutinyplot.get_data("Variable1")
        >>> metadata = mutinyplot.get_data("Variable1", metadata=True)
    ...
    if data_quant_in is not None:
        ...
        if name not in mutinyplot.data_quants.keys():
            logging.info("The name " + str(name) + " is currently not in mutinyplot")
            return
    
        temp_data_quant = mutinyplot.data_quants[name]
...
def get(name, xarray=False, metadata=False, dt=True, units=True):
    ...
    Parameters:
        ...
        dt : bool
            Return the times as np.datetime64[ns] objects instead of unix times
            (significantly faster); defaults to True for mutinyplot.get
        units: bool
            Attach the astropy units to the data and dependencioes prior to returning
            defaults to True for mutinyplot.get

    Returns: tuple of data/dimensions/metadata stored in mutinyplot
    ...
    Examples:
        ...
        >>> time, data = mutinyplot.get("Variable1")
