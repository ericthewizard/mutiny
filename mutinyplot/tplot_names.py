...
def tplot_names(quiet=False):
    ...
    for key, _ in mutinyplot.data_quants.items():
        if isinstance(mutinyplot.data_quants[key], dict):
        ...
        if len(mutinyplot.data_quants[key].attrs['plot_options']['overplots_mpl']) != 0:
            if quiet:
                # In this context we only want variable names, and no other formatting
                names_to_print = mutinyplot.data_quants[key].name
            else:
                names_to_print = mutinyplot.data_quants[key].name + "  data from: "
                for oplot_name in mutinyplot.data_quants[key].attrs['plot_options']['overplots_mpl']:
