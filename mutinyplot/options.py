...
def options(name, option=None, value=None, opt_dict=None):
    ...
    --------
        ...
        >>> mutinyplot.options('Variable1', 'yrange', [2,4])

        >>> # Change Variable1 to use a log scale
        >>> mutinyplot.options('Variable1', 'ylog', 1)
        >>> mutinyplot.tplot('Variable1')

        >>> # Multi-dimensional variable
        >>> y_data = np.random.rand(5, 4, 3)
        >>> v1_data = [0, 1, 3, 4]
        >>> v2_data = [1, 2, 3]
        >>> mutinyplot.store_data("Variable2", data={'x': x_data, 'y': y_data, 'v1': v1_data, 'v2': v2_data})
        >>> # Set the spectrogram plots to show dimension 'v2' at slice 'v1' = 0
        >>> mutinyplot.options('Variable2', 'spec', 1)
        >>> mutinyplot.options("Variable2", "spec_dim_to_plot", 'v2')
        >>> mutinyplot.options("Variable2", "spec_slices_to_use", {'v1': 0})
        >>> mutinyplot.tplot('Variable2')

    """

    if isinstance(name, int):
        name = list(mutinyplot.data_quants.keys())[name]
    ...
    for i in name:
        ...
        for option, value in opt_dict.items():
            ...
            if i not in mutinyplot.data_quants.keys():
                logging.info(str(i) + " is currently not in mutinyplot.")
                return

            if option == 'color':
                if isinstance(value, list):
                    mutinyplot.data_quants[i].attrs['plot_options']['extras']['line_color'] = value
                else:
                    mutinyplot.data_quants[i].attrs['plot_options']['extras']['line_color'] = [value]

            if option == 'link':
                if isinstance(value, list):
                    mutinyplot.link(i, value[1], value[0])

            if option == 'colormap':
                if isinstance(value, list):
                    mutinyplot.data_quants[i].attrs['plot_options']['extras']['colormap'] = value
                else:
                    mutinyplot.data_quants[i].attrs['plot_options']['extras']['colormap'] = [value]

            if option == 'colormap_width':
                mutinyplot.data_quants[i].attrs['plot_options']['extras']['colormap_width'] = value

            if option == 'second_axis_size':
                mutinyplot.data_quants[i].attrs['plot_options']['extras']['second_axis_size'] = value

            if option == 'spec':
                _reset_plots(i)
                if value:
                    if 'spec_bins' not in mutinyplot.data_quants[i].coords:
                        logging.warning(f"{i} does not contain coordinates for spectrogram plotting.  Continuing...")
                        continue
                    else:
                        mutinyplot.data_quants[i].attrs['plot_options']['extras']['spec'] = value
                        mutinyplot.data_quants[i].attrs['plot_options']['yaxis_opt']['y_range'] = utilities.get_y_range(mutinyplot.data_quants[i])
                ...
                if 'spec_dim_to_plot' not in mutinyplot.data_quants[i].attrs['plot_options']['extras']:
                    if 'v' in mutinyplot.data_quants[i].coords:
                        mutinyplot.data_quants[i].attrs['plot_options']['extras']['spec_dim_to_plot'] = 'v'
                    elif 'v2' in mutinyplot.data_quants[i].coords:
                        mutinyplot.data_quants[i].attrs['plot_options']['extras']['spec_dim_to_plot'] = 'v2'
                    else:
                        mutinyplot.data_quants[i].attrs['plot_options']['extras']['spec_dim_to_plot'] = 'v1'

            if option == 'alt':
                _reset_plots(i)
                mutinyplot.data_quants[i].attrs['plot_options']['extras']['alt'] = value

            if option == 'map':
                _reset_plots(i)
                mutinyplot.data_quants[i].attrs['plot_options']['extras']['map'] = value

            if option == 'legend_names' or option == 'labels':
                if isinstance(value, list):
                    mutinyplot.data_quants[i].attrs['plot_options']['yaxis_opt']['legend_names'] = value
                else:
                    mutinyplot.data_quants[i].attrs['plot_options']['yaxis_opt']['legend_names'] = [value]

            if option == 'legend_location' or option == 'labels_location':
                mutinyplot.data_quants[i].attrs['plot_options']['yaxis_opt']['legend_location'] = value

            if option == 'legend_size' or option == 'labels_size':
                mutinyplot.data_quants[i].attrs['plot_options']['yaxis_opt']['legend_size'] = value

            if option == 'legend_shadow' or option == 'labels_shadow':
                mutinyplot.data_quants[i].attrs['plot_options']['yaxis_opt']['legend_shadow'] = value

            if option == 'legend_title' or option == 'labels_title':
                mutinyplot.data_quants[i].attrs['plot_options']['yaxis_opt']['legend_title'] = value

            if option == 'legend_titlesize' or option == 'labels_titlesize':
                mutinyplot.data_quants[i].attrs['plot_options']['yaxis_opt']['legend_titlesize'] = value

            if option == 'legend_color' or option == 'labels_color':
                mutinyplot.data_quants[i].attrs['plot_options']['yaxis_opt']['legend_color'] = value

            if option == 'legend_edgecolor' or option == 'labels_edgecolor':
                mutinyplot.data_quants[i].attrs['plot_options']['yaxis_opt']['legend_edgecolor'] = value

            if option == 'legend_facecolor' or option == 'labels_facecolor':
                mutinyplot.data_quants[i].attrs['plot_options']['yaxis_opt']['legend_facecolor'] = value

            if option == 'legend_markerfirst' or option == 'labels_markerfirst':
                mutinyplot.data_quants[i].attrs['plot_options']['yaxis_opt']['legend_markerfirst'] = value

            if option == 'legend_markerscale' or option == 'labels_markerscale':
                mutinyplot.data_quants[i].attrs['plot_options']['yaxis_opt']['legend_markerscale'] = value

            if option == 'legend_markersize' or option == 'legend_markersize':
                mutinyplot.data_quants[i].attrs['plot_options']['yaxis_opt']['legend_markersize'] = value

            if option == 'legend_frameon' or option == 'labels_frameon':
                mutinyplot.data_quants[i].attrs['plot_options']['yaxis_opt']['legend_markerscale'] = value

            if option == 'legend_ncols' or option == 'labels_ncols':
                mutinyplot.data_quants[i].attrs['plot_options']['yaxis_opt']['legend_ncols'] = value

            if option == 'xlog_slice':
                if value:
                    mutinyplot.data_quants[i].attrs['plot_options']['slice_xaxis_opt']['xi_axis_type'] = 'log'
                else:
                    mutinyplot.data_quants[i].attrs['plot_options']['slice_xaxis_opt']['xi_axis_type'] = 'linear'

            if option == 'ylog':
                negflag = 0 # _ylog_check(data_quants, value, i)
                if negflag == 0 and value:
                    mutinyplot.data_quants[i].attrs['plot_options']['yaxis_opt']['y_axis_type'] = 'log'
                else:
                    mutinyplot.data_quants[i].attrs['plot_options']['yaxis_opt']['y_axis_type'] = 'linear'

            if option == 'ylog_slice':
                if value:
                    mutinyplot.data_quants[i].attrs['plot_options']['slice_yaxis_opt']['yi_axis_type'] = 'log'
                else:
                    mutinyplot.data_quants[i].attrs['plot_options']['slice_yaxis_opt']['yi_axis_type'] = 'linear'

            if option == 'zlog':
                # check for negative values and warn the user that they will be ignored
                negflag = _zlog_check(mutinyplot.data_quants, value, i)
                if negflag != 0 and value:
                ...
                if value:
                    mutinyplot.data_quants[i].attrs['plot_options']['zaxis_opt']['z_axis_type'] = 'log'
                else:
                    mutinyplot.data_quants[i].attrs['plot_options']['zaxis_opt']['z_axis_type'] = 'linear'

            if option == 'nodata':
                mutinyplot.data_quants[i].attrs['plot_options']['line_opt']['visible'] = value
            ...
            if option == 'line_style' or option == 'linestyle':
                ...
                mutinyplot.data_quants[i].attrs['plot_options']['line_opt']['line_style'] = to_be

                mutinyplot.data_quants[i].attrs['plot_options']['line_opt']['line_style_name'] = _convert_to_matplotlib_linestyle(value)

                if(value == 6 or value == 'none'):
                    mutinyplot.data_quants[i].attrs['plot_options']['line_opt']['visible'] = False

            if option == 'char_size' or option == 'charsize':
                mutinyplot.data_quants[i].attrs['plot_options']['extras']['char_size'] = value

            if option == 'name':
                mutinyplot.data_quants[i].attrs['plot_options']['line_opt']['name'] = value

            if option == 'title':
                mutinyplot.data_quants[i].attrs['plot_options']['line_opt']['title'] = value

            if option == "panel_size":
                if value > 1 or value <= 0:
                    logging.info("Invalid panel_size value (%f). Should be in (0, 1]",value)
                    return
                mutinyplot.data_quants[i].attrs['plot_options']['extras']['panel_size'] = value

            if option == 'basemap':
                mutinyplot.data_quants[i].attrs['plot_options']['extras']['basemap'] = value

            if option == 'alpha':
                if value > 1 or value < 0:
                    logging.info("Invalid alpha value (%f). Should be [0, 1]",value)
                    return
                mutinyplot.data_quants[i].attrs['plot_options']['extras']['alpha'] = value

            if option == 'marker':
                mutinyplot.data_quants[i].attrs['plot_options']['line_opt']['marker'] = value

            if option == 'errorevery':
                mutinyplot.data_quants[i].attrs['plot_options']['line_opt']['errorevery'] = value

            if option == 'capsize':
                mutinyplot.data_quants[i].attrs['plot_options']['line_opt']['capsize'] = value

            if option == 'ecolor':
                mutinyplot.data_quants[i].attrs['plot_options']['line_opt']['ecolor'] = value

            if option == 'elinewidth':
                mutinyplot.data_quants[i].attrs['plot_options']['line_opt']['elinewidth'] = value

            if option == 'marker_size':
                mutinyplot.data_quants[i].attrs['plot_options']['line_opt']['marker_size'] = value

            if option == 'markevery':
                mutinyplot.data_quants[i].attrs['plot_options']['line_opt']['markevery'] = value

            if option == 'symbols':
                mutinyplot.data_quants[i].attrs['plot_options']['line_opt']['symbols'] = value

            if option == 'xtick_length':
                mutinyplot.data_quants[i].attrs['plot_options']['extras']['xtick_length'] = value

            if option == 'ytick_length':
                mutinyplot.data_quants[i].attrs['plot_options']['extras']['ytick_length'] = value

            if option == 'xtick_width':
                mutinyplot.data_quants[i].attrs['plot_options']['extras']['xtick_width'] = value

            if option == 'ytick_width':
                mutinyplot.data_quants[i].attrs['plot_options']['extras']['ytick_width'] = value

            if option == 'xtick_color':
                mutinyplot.data_quants[i].attrs['plot_options']['extras']['xtickcolor'] = value

            if option == 'ytick_color':
                mutinyplot.data_quants[i].attrs['plot_options']['extras']['ytickcolor'] = value

            if option == 'xtick_labelcolor':
                mutinyplot.data_quants[i].attrs['plot_options']['extras']['xtick_labelcolor'] = value

            if option == 'ytick_labelcolor':
                mutinyplot.data_quants[i].attrs['plot_options']['extras']['ytick_labelcolor'] = value

            if option == 'xtick_direction':
                mutinyplot.data_quants[i].attrs['plot_options']['extras']['xtick_direction'] = value

            if option == 'ytick_direction':
                mutinyplot.data_quants[i].attrs['plot_options']['extras']['ytick_direction'] = value

            if option == 'right_axis':
                mutinyplot.data_quants[i].attrs['plot_options']['extras']['right_axis'] = value

            if option == 'thick':
                if isinstance(value, list):
                    mutinyplot.data_quants[i].attrs['plot_options']['line_opt']['line_width'] = value
                else:
                    mutinyplot.data_quants[i].attrs['plot_options']['line_opt']['line_width'] = [value]

            if option == 'yrange' or option == 'y_range':
                mutinyplot.data_quants[i].attrs['plot_options']['yaxis_opt']['y_range'] = [value[0], value[1]]
                # track whether the yrange option was set by the user
                mutinyplot.data_quants[i].attrs['plot_options']['yaxis_opt']['y_range_user'] = True

            if option == 'y_major_ticks':
                # check whether the value is 1D array-like
                if isinstance(value, (list, np.ndarray)):
                    mutinyplot.data_quants[i].attrs['plot_options']['yaxis_opt']['y_major_ticks'] = value
                else:
            ...
            if option == 'y_minor_tick_interval':
                ...
                if isinstance(value, (int, float)):
                    mutinyplot.data_quants[i].attrs['plot_options']['yaxis_opt']['y_minor_tick_interval'] = value
                else:
                    logging.warning('y_minor_tick_interval must be a number')

            if option == 'zrange' or option == 'z_range':
                mutinyplot.data_quants[i].attrs['plot_options']['zaxis_opt']['z_range'] = [value[0], value[1]]

            if option == 'xrange_slice':
                plt_opts = mutinyplot.data_quants[i].attrs['plot_options']
            ...
            if option == 'xtitle':
                mutinyplot.data_quants[i].attrs['plot_options']['xaxis_opt']['axis_label'] = value

            if option == 'ytitle':
                mutinyplot.data_quants[i].attrs['plot_options']['yaxis_opt']['axis_label'] = value

            if option == 'ztitle':
                mutinyplot.data_quants[i].attrs['plot_options']['zaxis_opt']['axis_label'] = value

            if option == 'xsubtitle':
                mutinyplot.data_quants[i].attrs['plot_options']['xaxis_opt']['axis_subtitle'] = value

            if option == 'ysubtitle':
                mutinyplot.data_quants[i].attrs['plot_options']['yaxis_opt']['axis_subtitle'] = value

            if option == 'zsubtitle':
                mutinyplot.data_quants[i].attrs['plot_options']['zaxis_opt']['axis_subtitle'] = value

            if option == 'ytitle_color':
                mutinyplot.data_quants[i].attrs['plot_options']['yaxis_opt']['axis_color'] = value

            if option == 'ztitle_color':
                mutinyplot.data_quants[i].attrs['plot_options']['zaxis_opt']['axis_color'] = value

            if option == 'ybar':
                mutinyplot.data_quants[i].attrs['plot_options']['extras']['ybar'] = value

            if option == 'ybar_color':
                mutinyplot.data_quants[i].attrs['plot_options']['extras']['ybar'] = value

            if option == 'ybar_size':
                mutinyplot.data_quants[i].attrs['plot_options']['extras']['ysize'] = value

            if option == 'plotter':
                _reset_plots(i)
                mutinyplot.data_quants[i].attrs['plot_options']['extras']['plotter'] = value

            if option == 'crosshair_x':
                mutinyplot.data_quants[i].attrs['plot_options']['xaxis_opt']['crosshair'] = value

            if option == 'crosshair_y':
                mutinyplot.data_quants[i].attrs['plot_options']['yaxis_opt']['crosshair'] = value

            if option == 'crosshair_z':
                mutinyplot.data_quants[i].attrs['plot_options']['zaxis_opt']['crosshair'] = value

            if option == 'static':
                mutinyplot.data_quants[i].attrs['plot_options']['extras']['static'] = value

            if option == 'static_tavg':
                mutinyplot.data_quants[i].attrs['plot_options']['extras']['static_tavg'] = [value[0], value[1]]

            if option == 't_average':
                mutinyplot.data_quants[i].attrs['plot_options']['extras']['t_average'] = value

            if option == 'data_gap': #jmm, 2023-06-20
                mutinyplot.data_quants[i].attrs['plot_options']['extras']['data_gap'] = value

            if option == 'spec_dim_to_plot' or option == 'spec_plot_dim':
                if len(mutinyplot.data_quants[i].values.shape) <= 2:
                    logging.warning(f"Must have more than 2 coordinate dimensions to set spec_coord_to_plot for {mutinyplot.data_quants[i].name}")
                    continue

                # Set the 'spec_dim_to_plot' value to either 'v' or 'v1', 'v2', 'v3', etc.
                if isinstance(value, int):
                    coord_to_plot = "v" + str(value)
                    if coord_to_plot not in mutinyplot.data_quants[i].coords:
                        if value == 1:
                            coord_to_plot = "v"
                            if coord_to_plot not in mutinyplot.data_quants[i].coords:
                                logging.warning(f"Dimension {value} not found in {mutinyplot.data_quants[i].name}")
                                continue
                        else:
                            logging.warning(f"Dimension {value} not found in {mutinyplot.data_quants[i].name}")
                            continue
                    mutinyplot.data_quants[i].attrs['plot_options']['extras']['spec_dim_to_plot'] = coord_to_plot
                    ...
                    if coord_to_plot not in mutinyplot.data_quants[i].coords:
                        ...
                        mutinyplot.data_quants[i].attrs['plot_options']['extras']['spec_dim_to_plot'] = value

                # If we're plotting against different coordinates, we need to change what we consider the "spec_bins"
                mutinyplot.data_quants[i].coords['spec_bins'] = mutinyplot.data_quants[i].coords[coord_to_plot]
                mutinyplot.data_quants[i].attrs['plot_options']['yaxis_opt']['y_range'] = utilities.get_y_range(mutinyplot.data_quants[i])
            ...
            if option == 'spec_slices_to_use':
                if not isinstance(value, dict):
                    ...
                    for coord in value:
                        if coord not in mutinyplot.data_quants[i].coords:
                            logging.warning(f"Dimension {coord} not found in {mutinyplot.data_quants[i].name}")
                            continue

                mutinyplot.data_quants[i].attrs['plot_options']['extras']['spec_slices_to_use'] = value

            if option == 'border':
                mutinyplot.data_quants[i].attrs['plot_options']['extras']['border'] = value

            if option == 'var_label_ticks':
                mutinyplot.data_quants[i].attrs['plot_options']['var_label_ticks'] = value

            if option == 'y_interp':
                mutinyplot.data_quants[i].attrs['plot_options']['yaxis_opt']['y_interp'] = value

            if option == 'y_interp_points':
                mutinyplot.data_quants[i].attrs['plot_options']['yaxis_opt']['y_interp_points'] = value

            if option == 'x_interp':
                mutinyplot.data_quants[i].attrs['plot_options']['yaxis_opt']['x_interp'] = value

            if option == 'x_interp_points':
                mutinyplot.data_quants[i].attrs['plot_options']['yaxis_opt']['x_interp_points'] = value
            if option == 'y_no_resample':
                mutinyplot.data_quants[i].attrs['plot_options']['yaxis_opt']['y_no_resample'] = value
    return
...
def _reset_plots(name):
    if isinstance(mutinyplot.data_quants[name], dict):  # non-record varying variable
        return
    mutinyplot.data_quants[name].attrs['plot_options']['extras']['spec'] = 0
    mutinyplot.data_quants[name].attrs['plot_options']['extras']['alt'] = 0
    mutinyplot.data_quants[name].attrs['plot_options']['extras']['map'] = 0
    mutinyplot.data_quants[name].attrs['plot_options']['extras']['plotter'] = None
